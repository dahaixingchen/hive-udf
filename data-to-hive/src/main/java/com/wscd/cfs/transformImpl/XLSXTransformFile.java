 ...
 * @Version 1.0
 **/
public class XLSXTransformFile implements TransformFile {
    private static Logger logger = Logger.getLogger(XLSXTransformFile.class);
    ...
    public XLSXTransformFile(String inputFloderPath, String inputFileName,List<Map<String, Integer>> hiveColumns) {
    ...
    private void getFileColumnOrder(Row firstRow, int firstCellIndex, int lastCellIndex, ArrayList<Integer> fileIndexList) {
        ...
        for (int i =0; i < hiveColumns.size()-1;i++){
            ...
            for (; cIndex < lastCellIndex; cIndex++) {
                ...
                if (cell.toString() != null && hiveColumns.get(i).containsKey(cell.toString().toLowerCase())){
                    //如果能匹配到，记录文件字段的位置
